...
public class Grid {
  ...
  private static int[][] initializeValues(int width, int height){
    // Initialise toutes les valeurs de la grille à zéro
  ...
  void resetGrid(){
    // Permet de réinitialiser toutes les valeurs de la grille à zéro
  ...
  int updateGrid(int position, int numeroPlayer, gamePackage.interfacePackage.WriteInLog write){
    ...
    for(int k = 0; k<height; k++){
      // Si la cellule est vide alors le joueur peut jouer dedans, sinon on regarde la cellule au dessus (toujours dans la colonne donnée)
    ...
    position = -3; // Move invalide, on doit recommencer (car colonne remplie)
